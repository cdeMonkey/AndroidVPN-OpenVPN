...
public class MainFragment extends Fragment implements View.OnClickListener, ChangeServer {
    ...
    private void startVpn() {
        try {
            // .ovpn file this is where you can change .ovpn file
            /*if(server.getOvpn().contains("us.ovpn"))
            {
                String ovpn = "";

                try {

                    ovpn = getOVPN("https://raw.githubusercontent.com/cdeMonkey/Openvpn/master/filedetails.json");
                } catch (Exception e) {
                    e.printStackTrace();
                }
                OpenVpnApi.startVpn(getContext(), ovpn, server.getCountry(), server.getOvpnUserName(), server.getOvpnUserPassword());
            ...
            //config is ovpn config found in .ovpn file
